...
Devices are matched by **VID/PID, not by name** — Windows reports most of them as the generic "USB Input Device", and the real product string lives in a bus-reported property the PnP enumerator doesn't key on. Adding a device is one row in `usbdev.KnownDevices`.

**Take those IDs from `usb scan`, never from the joystick registry.** The shipped wheelbase entry was originally `0x0483/0x0522` ("SIMAGIC Alpha Series Wheelbase"), read from `HKLM\...\MediaProperties\PrivateProperties\Joystick\OEM` while the base happened to be unplugged. That key lists every device ever attached and never forgets one, so the ID named a base this rig no longer has and the real one (a SIMAGIC Alpha EVO Pro, `0x3670/0x0501`) reported `not connected` while plugged in and working — fixed 2026-09-11, along with adding the `rim` entry for the Neo X that the same walk turned up. The failure is silent by construction: an ID matching no hardware is indistinguishable from a device that is genuinely unplugged. `TestKnownDevicesAreWellFormed` catches duplicate aliases and duplicate hardware IDs but cannot tell whether an ID is real. Instance IDs carrying `&MI_` are rejected: a composite device like the MOZA handbrake publishes one node per USB interface (a `COM6` serial port and a game controller) under a single top-level node, and it is the top-level node that gets toggled.
...
- `shaker` can only verify the path from PC to the audio interface — that Windows accepted the format, opened the device and played the buffers. It cannot detect whether the transducer moved, and nothing in software can tell you an amplifier is electrically sound. The summary says so rather than reporting a clean run as "working". **Confirmed the hard way on 2026-09-11**: after the ButtKicker smelled of burning and went silent, the `KT USB Audio` device still enumerated as entirely healthy (every interface `OK`) and `shaker test` still reported clean runs. The USB audio interface and the power amplifier are separate sections and only the first is visible to software — never infer amplifier health from this command
